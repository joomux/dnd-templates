...
def draw_npc_page_1(c):
    """Draw the first page of NPC tracking."""
    ...
    draw_field_with_label(c, MARGIN + 5, y, "Appearance:",
                          MARGIN + 100, content_width + MARGIN - 5)
    y -= 18

    draw_field_with_label(c, MARGIN + 5, y, "Voice/Quirks:",
    ...
    # === FACTION & LOCATION SECTION ===
    y = draw_section_header(c, MARGIN, y, content_width, "Affiliations")
    y -= 5

    draw_field_with_label(c, MARGIN + 5, y, "Faction:",
                          MARGIN + 100, content_width + MARGIN - 5)
    y -= 18

    draw_field_with_label(c, MARGIN + 5, y, "Current Location:",
                          MARGIN + 75, content_width + MARGIN - 5)
    y -= 18

    y -= 8
    c.setFont("Helvetica", 9)
    c.setFillColor(MED_GRAY)
    c.drawString(MARGIN + 5, y, "Relationships:")
    y -= 12
    y = draw_ruled_lines(c, MARGIN + 10, y, content_width - 15, 3)

    y -= 10

    # === SESSIONS & NOTES SECTION ===
    y = draw_section_header(c, MARGIN, y, content_width, "Campaign Tracking")
    y -= 5

    draw_field_with_label(c, MARGIN + 5, y, "Sessions Appeared In:",
    ...
    c.drawString(MARGIN + 5, y, "Notes:")
    y -= 12
    y = draw_ruled_lines(c, MARGIN + 10, y, content_width - 15, 5)

    return y

def draw_npc_page_2(c):
    """Draw the second page of NPC tracking (extended notes)."""
    page_width = RM2_WIDTH
    content_width = page_width - 2 * MARGIN
    ...
    # === BACKSTORY & MOTIVATIONS SECTION ===
    y = draw_section_header(c, MARGIN, y, content_width, "Backstory & Motivations")
    y -= 5
    y = draw_ruled_lines(c, MARGIN + 5, y, content_width - 10, 8)

    y -= 10

    # === CAMPAIGN IMPACT SECTION ===
    y = draw_section_header(c, MARGIN, y, content_width, "Campaign Impact")
    y -= 5
    y = draw_ruled_lines(c, MARGIN + 5, y, content_width - 10, 8)
    ...
    y = draw_section_header(c, MARGIN, y, content_width, "Future Appearances & Plot Hooks")
    y -= 5
    y = draw_ruled_lines(c, MARGIN + 5, y, content_width - 10, 8)
...
def create_npc_template(filename="npc_template.pdf"):
    """Generate the NPC template (2 pages)."""
